...
import DataOnDisk from './DataOnDisk';
...
const dataOnDisk = new DataOnDisk();

export const fetchNotes = async () => {
	try {
		const fromTS = store.state.sync.lastModified;
		const latestNotes = await dataOnDisk.getLatestNotes(fromTS);
		const categories = await dataOnDisk.getCategoriesIfChangedSince(fromTS);
		const settings = await dataOnDisk.getSettings();
		store.commit('setSettings', settings);
		if (categories) {
		...
		if (latestNotes) {
			store.dispatch('updateNotes', {
				noteIds: latestNotes.noteIds,
				notes: latestNotes.notes
			});
			store.commit('setSyncLastModified', latestNotes.lastModified);
		}
		if (!categories && !latestNotes) {
			return;
		} else {
			const lastViewedNote = await dataOnDisk.getLastViewedNote();
			return { lastViewedNote };
		}
	} catch (err) {
		console.error(err);
		showError(t('notes', 'Fetching notes has failed.'));
	}
}
